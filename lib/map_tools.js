...
export function processStationRecords(records) {
    ...
    for (let record of records) {
        ...
        let weekIndex = getWeekNumber(record['Start Time']);
...
export function getWeeklyTotals(records) {
    ...
    for (let record of records) {
        ...
        if (weeklyAverages[weekIndex] === undefined) {
            weeklyAverages[weekIndex] = [];
        }

        weeklyAverages[weekIndex].push(record);
    }

    for (let week = 0; week < weeklyAverages.length; week++) {
        if (weeklyAverages[week] === undefined) {
            continue;
        }
        
        weeklyAverages[week] = weeklyAverages[week].length;
    }
