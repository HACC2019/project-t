...
class RecordAnalytics {
   ...
   * 
   */
  getInvalidSessions(stationID, hours) {
    const data = 0;
  }
}
...
getByDataByPayType(stationID, months, payMeth) {
...
getByDataBySessionStart(stationID, months, sessionStart) {
  if(sessionStart !== "MOBILE" || sessionStart !== "DEVICE") {
    return -1
  }
  else {
    let data = getRecords(stationID, months);
