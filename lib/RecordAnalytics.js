...
class RecordAnalytics {
  ...
  constructor(timeSimulation) {
    ...
    this._futureTimeRange = {on: false, type: 'Current', value: 0};
  ...
  /**
   * Sets the time range of the forecasting functions
   * @param {string} type Either Month or Hour
   * @param {number} Number of time units
   */
  setFutureTimeRange(type, value, aggregate) {
    this._futureTimeRange = {type: type.substring(0, 1).toUpperCase() + type.substring(1).toLowerCase(), value, aggregate: Number(aggregate)};
    for (let [thisValue, listeners] of this._listeners) {
      for (let callback of listeners) {
        callback.apply(thisValue);
      }
